...
const getMetaCatalog = async (req, res) => {
  try {
    ...
    if (!products || products.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No products found for catalog",
      });
    }

    // Filter only in-stock products
    const inStockProducts = products.filter((p) => p.finalStock > 0);

    // Transform products into Meta Catalog format
    const feed = inStockProducts.map((p) => {
      const productObj = {
        id: p.productId || p._id.toString(),
        title: p.name,
        description: p.shortDesc || p.name || "No description available",
        availability: "in stock",
        condition: "new",
        price: `${(p.finalPrice - (p.finalDiscount || 0)).toFixed(2)} BDT`,
        link: `${process.env.CLIENT_URL}/product/${p.slug}`, // single product URL
        brand: p.brand || "PerfectFitBD",
        category: p.category?.name || "Uncategorized",
      };

      // Only add image_link if thumbnail exists
      if (p.thumbnailImage) {
        productObj.image_link = `${process.env.BACKEND_URL}/uploads/${p.thumbnailImage}`;
      }
    ...
    res.status(500).json({
      success: false,
      message: "Failed to generate catalog feed",
      error: error.message,
    });
  }
};
